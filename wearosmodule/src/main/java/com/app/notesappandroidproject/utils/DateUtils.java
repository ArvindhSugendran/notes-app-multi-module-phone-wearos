...
import com.app.notesappandroidproject.WatchApp;
    ...
    public static String formatDateFromTimeString(long time) {
        ...
        if (totalSec > 0) {
            ...
            if (day >= 30) {
                ...
                result = ((int) day) + SPACE + WatchApp.getInstance().getString(R.string.lb_day_ago);
            } else if (day > 0) {
                result = WatchApp.getInstance().getString(R.string.lb_yesterday);
            } else {
                hour = (int) (totalSec / ONE_HOUR);
                if (hour > 0) {
                    result = hour + SPACE + WatchApp.getInstance().getString(R.string.lb_hour_ago);
                } else {
                    min = (int) (totalSec) / ONE_MINUTE;
                    if (min == 0) {
                        result = WatchApp.getInstance().getString(R.string.lb_just_finished);
                    } else {
                        result = min + SPACE + WatchApp.getInstance().getString(R.string.lb_minute_ago);
                    }
                }
            }
        }
